...
function makeTextEntry$2(id, labelKey, prop) {
    return function Entry(props) {
        ...
        return propertiesPanel.TextFieldEntry({ element, id, label: translate(labelKey), getValue, setValue, debounce });
    };
}
...
function AssignmentDefinitionProps(element) {
    if (element.type !== 'bpmn:UserTask')
        return [];
    return [
        { id: 'zenbpm-assign-assignee', component: AssigneeEntry, isEdited: propertiesPanel.isTextFieldEntryEdited },
        { id: 'zenbpm-assign-candidateGroups', component: CandidateGroupsEntry, isEdited: propertiesPanel.isTextFieldEntryEdited },
    ];
}
...
function TaskScheduleProps(element) {
    if (element.type !== 'bpmn:UserTask')
        return [];
    return [
        { id: 'zenbpm-schedule-dueDate', component: DueDateEntry, isEdited: propertiesPanel.isTextFieldEntryEdited },
        { id: 'zenbpm-schedule-followUpDate', component: FollowUpDateEntry, isEdited: propertiesPanel.isTextFieldEntryEdited },
    ];
}
...
function CalledElementProps(element) {
    if (element.type !== 'bpmn:CallActivity')
        return [];
    return [
        { id: 'zenbpm-calledEl-processId', component: ProcessIdEntry, isEdited: propertiesPanel.isTextFieldEntryEdited },
        { id: 'zenbpm-calledEl-propagateAllChildVariables', component: PropagateAllChildVarsEntry, isEdited: propertiesPanel.isToggleSwitchEntryEdited },
        { id: 'zenbpm-calledEl-propagateAllParentVariables', component: PropagateAllParentVarsEntry, isEdited: propertiesPanel.isToggleSwitchEntryEdited },
    ];
}
...
function CalledDecisionProps(element) {
    if (element.type !== 'bpmn:BusinessRuleTask')
        return [];
    return [
        { id: 'zenbpm-calledDecision-decisionId', component: DecisionIdEntry, isEdited: propertiesPanel.isTextFieldEntryEdited },
        { id: 'zenbpm-calledDecision-resultVariable', component: ResultVariableEntry, isEdited: propertiesPanel.isTextFieldEntryEdited },
    ];
}
...
function createInputMappingGroup(element, injector) {
    ...
    const items = inputs.map((input, index) => {
        ...
        return {
            ...
            entries: [
                { id: `${id}-source`, component: makeParamEntry(`${id}-source`, 'Source expression', 'source', element, input), isEdited: propertiesPanel.isTextFieldEntryEdited },
...
function createOutputMappingGroup(element, injector) {
    ...
    const items = outputs.map((output, index) => {
        ...
        return {
            ...
            entries: [
                { id: `${id}-source`, component: makeParamEntry(`${id}-source`, 'Source expression', 'source', element, output), isEdited: propertiesPanel.isTextFieldEntryEdited },
